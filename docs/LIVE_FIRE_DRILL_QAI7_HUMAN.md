...
| H9b | 同 session：`plan "请执行上面的计划" --session-id h8a`（或 `run --plan-id`） | 装配层 `run_plan` 原 `plan_id`，不新 Hybrid | `mode_notice`：`You asked to run plan_xxx from Plan mode.` + `executing` / `result`。`--human` 可见 `N items`，无 id 全文。一次性 `run "<新问句>"` 会新规划 | ☐ | ☐ Confirm 可选 |  |
