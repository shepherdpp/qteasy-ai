...
def execute_magic_command(
    ...
    *,
    ...
    if command.diag:
        diagnostics = assistant.debug_config()
        if command.response_style == "raw":
            return {
                "mode": "diag",
        ...
        narrative = (
            "Provider diagnostics collected from API path.\n"
            f"- provider_enabled: {diagnostics.get('provider_enabled')}\n"
            f"- model: {diagnostics.get('model', '')}\n"
            f"- base_url: {diagnostics.get('base_url', '')}\n"
            f"- timeout: {diagnostics.get('timeout', '')}\n"
            f"- api_key_present: {diagnostics.get('api_key_present')}"
        )
        return {
            "mode": "diag",
            "result": AssistantOutput(
                narrative=narrative,
                python_code="from qteasy_ai.app import QteasyAssistant\nassistant = QteasyAssistant()\nassistant.debug_config()",
                result_preview=str(diagnostics.get("config_sources", {})),
