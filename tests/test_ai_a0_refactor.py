...
class TestAiA0Refactor(unittest.TestCase):
    ...
    def test_config_center_reads_deprecated_qt_config_keys(self) -> None:
        ...
        qt_config = {
            "ai_model": "compat_model",
            "ai_api_key": "sk-test",
            "ai_base_url": "http://127.0.0.1:8000/v1",
            "ai_timeout": 33,
            "ai_home": ".qteasy/ai/compat_home",
        }
        center = ConfigCenter(env={}, qt_config=qt_config)
        provider_cfg = center.resolve_provider_config()
        home_val = center.resolve("ai_home", env_key="QTEASY_AI_HOME", qt_key="ai_home", default=".qteasy/ai/")
        print("\n[TestAiA0Refactor] deprecated qt_config provider cfg:", provider_cfg)
        print(" ai_home:", home_val)
        self.assertEqual(provider_cfg["model"], "compat_model")
        self.assertEqual(provider_cfg["base_url"], "http://127.0.0.1:8000/v1")
        self.assertEqual(provider_cfg["timeout"], 33)
        self.assertEqual(home_val, ".qteasy/ai/compat_home")

    def test_qt_configuration_and_config_center_consistency(self) -> None:
        """验证注入 qt_config 与 ConfigCenter 读取一致（不依赖 qteasy 全局 ai_* 键）。"""
        ...
        provider_cfg = center.resolve_provider_config()
        home_val = center.resolve("ai_home", env_key="QTEASY_AI_HOME", qt_key="ai_home", default=".qteasy/ai/")
        print("\n[TestAiA0Refactor] injected qt_config:", qt_config)
